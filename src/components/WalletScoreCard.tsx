...
const WalletScoreCard = ({
  ...
  const fetchAllTransactions = async (address: string, maxLimit = 1000) => {
    ...
    const [txData, activityData] = await Promise.all([getAccountTransactions(address, maxLimit), getAccountActivities(address, maxLimit)]);
  ...
  const calculateMetrics = (transactions: any[], activities: any[], apiTotalTx: number): ScoreMetrics => {
    ...
    allTxs.forEach(tx => {
      ...
      const isContractTx = tx.isContract || tx.contractAddress || tx.to && tx.to !== walletAddress && tx.to !== '' && tx.to !== '0x0000000000000000000000000000000000000000' || tx.transactionAddress && tx.transactionAddress !== walletAddress || tx.methodID || tx.methodName;
      if (isContractTx) {
        contractsInteracted++;
    ...
    // Time-based metrics
    const timestamps = allTxs.map(tx => tx.timestamp).filter(ts => ts).map(ts => typeof ts === 'number' && ts > 1e12 ? ts / 1000 : ts).sort((a, b) => a - b);
    ...
    const averageGasPrice = gasTransactions.length > 0 ? gasTransactions.reduce((sum, tx) => sum + Number(tx.gasPrice), 0) / gasTransactions.length / 1e9 : 0;
  ...
  const calculateScoreBreakdown = (metrics: ScoreMetrics): ScoreBreakdown => {
    ...
    const activityScore = Math.min(100, Math.min(50, metrics.totalTransactions * 0.1) + Math.min(50, metrics.transactionFrequency * 20));
    ...
    const consistencyScore = Math.min(100, Math.min(60, metrics.activeDays * 2) + Math.min(40, metrics.transactionFrequency * 30));

    // Diversity Score (0-100) - Based on unique contracts and interaction types
    const diversityScore = Math.min(100, Math.min(70, metrics.uniqueContracts * 2) + Math.min(30, metrics.diversityScore * 3));
  ...
  const calculateOverallScore = (breakdown: ScoreBreakdown): number => {
    ...
    return Math.round(breakdown.activity * weights.activity + breakdown.volume * weights.volume + breakdown.consistency * weights.consistency + breakdown.diversity * weights.diversity + breakdown.longevity * weights.longevity + breakdown.gasEfficiency * weights.gasEfficiency);
  };
  ...
  return <div className="space-y-6">
      ...
      <Card className={`${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-white/80 border-gray-200'} border-2`}>
        ...
        <CardContent>
          {loading ? <div className="text-center py-8">
              ...
              <button onClick={analyzeWallet} className="mt-4 px-4 py-2 bg-purple-600 text-white rounded hover:bg-purple-700">
                Retry Analysis
              </button>
            </div> : isEmpty ? <div className="text-center py-8">
              ...
              <div className={`space-y-3 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                ...
                <p className="max-w-md mx-auto">
                  {isLoreMode ? 'This consciousness has not yet materialized on the Monad blockchain. The mind remains dormant, waiting for its first digital awakening.' : 'This wallet has no transaction history on the Monad testnet. Start interacting with the blockchain to build your reputation score!'}
                </p>
              ...
              <div className="text-center">
                <div className="relative inline-block">
                  <div className={`text-6xl font-bold ${overallScore >= 70 ? 'text-green-400' : overallScore >= 50 ? 'text-blue-400' : overallScore >= 30 ? 'text-yellow-400' : overallScore >= 10 ? 'text-red-400' : 'text-gray-400'}`}>
                    {overallScore}
                  </div>
                  <div className={`text-2xl font-bold absolute -top-2 -right-8 ${overallScore >= 70 ? 'text-green-400' : overallScore >= 50 ? 'text-blue-400' : overallScore >= 30 ? 'text-yellow-400' : overallScore >= 10 ? 'text-red-400' : 'text-gray-400'}`}>
                    {scoreGrade}
                  </div>
                </div>
              ...
              {/* Score Breakdown */}
              {scoreBreakdown && <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                  {Object.entries(scoreBreakdown).map(([key, value]) => <div key={key} className={`p-4 rounded-lg ${isDarkMode ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
      ...
      {showArtGenerator && !loading && !error && !isEmpty && metrics && scoreBreakdown && <ReputationArtGenerator walletAddress={walletAddress} overallScore={overallScore} metrics={metrics} scoreBreakdown={scoreBreakdown} isDarkMode={isDarkMode} isLoreMode={isLoreMode} />}
    </div>;
};
